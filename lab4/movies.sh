...
function query_year () {
    # Returns list of movies from ${1} with value higher than ${2} in year slot
    local -r MOVIES_LIST=${1}
    local -r QUERY=${2}
    ...
    local RESULT_YEAR=""
    for MOVIE_FILE in ${MOVIES_LIST}; do
        RESULT_YEAR=$(grep "| Year" "${MOVIE_FILE}" | sed 's/| Year://')
        echo "${RESULT_YEAR}"
        if [[ "${RESULT_YEAR}" -gt "${QUERY}" ]]; then
            RESULTS_LIST+=( "${MOVIE_FILE}" )
        fi
    done
    echo "${RESULTS_LIST[@]:-}"
...
if ${IS_DIR_OPT:-false}; then
    if [[ ! -d ${MOVIES_DIR} ]]; then
        echo "${MOVIES_DIR} is not a catalog"
        exit 0
    fi
else
    echo "Provide -d option"
    exit 0
fi
...
if [[ -n "${FILE_4_SAVING_RESULTS}" ]]; then    
  ...
  if [[ "${EXTENSION}" == "${FILE_4_SAVING_RESULTS}" ]]; then
